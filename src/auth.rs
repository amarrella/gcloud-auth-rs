...
const CLOUDSDK_APPLICATION_DEFAULT_CLIENT_ID: &str =
...
const GOOGLE_OAUTH_TOKEN_URL: &str = concatcp!(GOOGLE_OAUTH_URL, "/token");
...
pub async fn idtoken_from_metadata_server(client: &reqwest::Client) -> Result<IdToken, Error> {
    let jwt = client
        .get(format!(
            "{GOOGLE_METADATA_URL}default/identity?audience={CLOUDSDK_CLIENT_ID}"
        ))
...
pub async fn accesstoken_from_metadata_server(
    ...
    let jwt = client
        .get(format!(
            "{GOOGLE_METADATA_URL}default/token?audience={CLOUDSDK_CLIENT_ID}"
        ))
        .header("Metadata-Flavor", "Google")
        .send()
        .await?
...
async fn get_active_account(config_path: &str) -> Result<String, String> {
    let active_config_path: String = format!("{config_path}/active_config");
    let active_config_binary = fs::read(&active_config_path)
        .await
        .expect(format!("Couldn't read active config from {active_config_path}").as_str());
    let active_config_str = String::from_utf8(active_config_binary)
        .expect("Couldn't decode config from {active_config_path}");
    let gcloud_config = ini::ini!(&format!(
        "{config_path}/configurations/config_{active_config_str}"
    ));
...
pub async fn get_idtoken(
    ...
    let idtoken = idtoken_from_metadata_server(&client).await;
    ...
    let result = match idtoken {
        ...
        Err(_) => {
            ...
            let credentials = read_credentials_from_db(connection, &active_account)
                .expect(format!("Couldn't find credentials at {active_account}").as_str());
...
pub async fn get_user_accesstoken(
    ...
    let accesstoken = accesstoken_from_metadata_server(&client).await;
...
pub async fn get_adc_accesstoken(client: &reqwest::Client) -> Result<AccessToken, Error> {
    let accesstoken = accesstoken_from_metadata_server(&client).await;
    ...
    let result = match accesstoken {
        ...
        Err(_) => {
            ...
            let credentials = read_credentials_from_file(&adc_path)
                .await
                .expect(format!("Couldn't find application default credentials").as_str());
            accesstoken_from_credentials(&credentials, &client).await
        }
    };
...
pub async fn installed_app_flow_auth(
    ...
    open::that(format!("{google_auth_url}"))
        .expect(format!("Couldn't open auth url {google_auth_url}").as_str());
    ...
    async fn get_auth_code(
        ...
        let code: Option<String> = caps.map(|cs| {
...
pub async fn application_default_login(
    ...
    let google_credentials_response = installed_app_flow_auth(
        client,
        CLOUDSDK_APPLICATION_DEFAULT_CLIENT_ID,
        CLOUDSDK_APPLICATION_DEFAULT_CLIENT_SECRET,
        &scopes,
    )
    .await;
    let credentials = GoogleCredentials {
        ...
        refresh_token: google_credentials_response?
            .refresh_token
    ...
    let credentials = serde_json::to_string(&credentials).expect("serialization error");
    fs::write(credentials_path, credentials)
        .await
        .expect("failed writing credentials");
    Ok(())
}
...
pub async fn user_login(
    ...
    let id_token = idtoken_from_credentials(&credentials, client)
        .await
        .unwrap();
    ...
    let google_keys = client
        ...
        .await?
        .error_for_status()?
        .json::<GoogleKeysResponse>()
        .await
        .unwrap();
    let google_key = google_keys
        .keys
        .iter()
        .find(|k| k.common.key_id == id_token_header.kid)
        .unwrap();
    ...
    let id_token_jwt =
        jsonwebtoken::decode::<GoogleClaims>(&id_token, &decoding_key, &validation).unwrap();
    let account = id_token_jwt.claims.email.unwrap();
    write_credentials_to_db(&connection, &account.as_str(), &credentials);
    Ok(())
}
